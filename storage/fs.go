...
func (backend *fsBackend) Put(name string, data []byte) (Version, error) {
	...
	for {
		...
		if err != nil {
			return 0, fmt.Errorf("failed to remove entry version '%s' (cause: %v)", removeFile, err)
		}
		versionCount--
	}
	...
	if err != nil {
		return 0, fmt.Errorf("failed to write entry version '%s' (cause: %v)", nextVersionFile, err)
	}
...
func (backend *fsBackend) List() (Names, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read storage path '%s' (cause: %v)", backend.path, err)
	}
...
func (backend *fsBackend) Get(name string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read entry version '%s' (cause: %v)", versionFile, err)
	}
	return data, nil
}
...
func (backend *fsBackend) checkEntryPath(name string, create bool) (string, error) {
	...
	if os.IsNotExist(err) {
		...
		if err != nil {
			return entryPath, fmt.Errorf("failed to create entry path '%s' (cause: %v)", entryPath, err)
		}
...
func (backend *fsBackend) readEntryVersions(entryPath string, ignoreEmpty bool) ([]Version, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read entry path '%s' (cause: %v)", entryPath, err)
	}
...
func checkFSStoragePath(path string) (string, error) {
	...
	if os.IsNotExist(err) {
		...
		if err != nil {
			return absolutePath, fmt.Errorf("failed to create storage path '%s' (cause: %v)", absolutePath, err)
		}
	} else if !pathInfo.IsDir() {
